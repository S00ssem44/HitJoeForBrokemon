# Hit Joe For Brokémon

# What is the game about?
This is a clicker game where you need to hit 'Joe' to get money, with your money you can buy Brokémon cards, Pack upgrades or hit upgrades.
Inside of those packs you can find rare cards sometimes that will give you permanent click boosts and passive income.
Can you collect them all?

## How to Play
1. Step 1, Click the link (https://s00ssem44.github.io/HitJoeForBrokemon/)
2. Step 2, Read Joes intro
3. Step 3, Click the button and for the rest you can just try it yourself!

(DISCLAIMER: If you find the secret cheat code in any way, if you use a number higher than around 100 for free pack upgrades, the game might crash...)
